...
pair<double, double> Search_positition_magnet(double dtime, vector<vector<vecter>>& area, vector<Magnet>& magnets, vector<Sensor>& sensors,			//!!!!BH_correct
	...
	for (int i = 0; i < MATRIX_OF_ALL_MAGNETS_POSITIONS_FOR_ALL_HALL_SENSORS.size(); ++i) {
		...
		while (!vecters_queue.empty() || num_of_all_cells != num_visit_cells || vecters_queue.top().second > MIN_DISTANCE_BTW_VECTERS) {
			...
			++num_visit_cells;
			for (int insert_elem_index = max(curr_position.first.first - 1, 0); insert_elem_index < min(x_length, curr_position.first.first + 1); ++insert_elem_index) { //!!!!BH_correct// where is length_x?
				for (int insert_elem_index_y = max(curr_position.first.second - 1, 0); insert_elem_index_y < min(x_length, curr_position.first.second + 1); ++insert_elem_index_y) {

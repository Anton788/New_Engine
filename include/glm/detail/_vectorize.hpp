...
namespace detail
{ 
	template <typename FF, typename T, precision P, template <typename, precision> class vecType>
	struct functor1{};

	template <typename FF, typename T, precision P>
	struct functor1<FF, T, P, tvec1>
	{
		GLM_FUNC_QUALIFIER static tvec1<FF, P> call(FF (*Func) (T x), tvec1<T, P> const & v)
		{
			return tvec1<FF, P>(Func(v.x));
		}
	};

	template <typename FF, typename T, precision P>
	struct functor1<FF, T, P, tvec2>
	{
		GLM_FUNC_QUALIFIER static tvec2<FF, P> call(FF (*Func) (T x), tvec2<T, P> const & v)
		{
			return tvec2<FF, P>(Func(v.x), Func(v.y));
		}
	};

	template <typename FF, typename T, precision P>
	struct functor1<FF, T, P, tvec3>
	{
		GLM_FUNC_QUALIFIER static tvec3<FF, P> call(FF (*Func) (T x), tvec3<T, P> const & v)
		{
			return tvec3<FF, P>(Func(v.x), Func(v.y), Func(v.z));
		}
	};

	template <typename FF, typename T, precision P>
	struct functor1<FF, T, P, tvec4>
	{
		GLM_FUNC_QUALIFIER static tvec4<FF, P> call(FF (*Func) (T x), tvec4<T, P> const & v)
		{
			return tvec4<FF, P>(Func(v.x), Func(v.y), Func(v.z), Func(v.w));
		}
	};

...
PermanentMagnet::PermanentMagnet() {
	...
	cin >> sight;
	Quater_mag_field.magnit = this;
	radius = diametr / 2;
}

PermanentMagnet::PermanentMagnet(int destiny_of_mag_pixels, double _strength, double _diametr, double _mass,
...
void PermanentMagnet::relocate(double dtime) {
...
void Magnet::show_field(int cherez_n) {
	...
	for (int x = Quater_mag_field.size_x / n - 1; x >= 0; x--)
	{
		cout << endl;
		for (int y = (Quater_mag_field.size_y / n - 1); y >= 0; y--)
		{

			if ((n*x < height / 2) && (n*y == diametr / 2))
				cout << "  III    ";
			else
				cout << Quater_mag_field.cells[n*x][n*y].Mag_vec.x_proj << "  ";
		}
	}
	...
	for (int x = Quater_mag_field.size_x / n - 1; x >= 0; x--)
	{
		...
		for (int y = (Quater_mag_field.size_y / n - 1); y >= 0; y--)
		{
			...
			if ((n*x < height / 2) && (n*y == diametr / 2))
				...
				cout << Quater_mag_field.cells[n*x][n*y].Mag_vec.y_proj << "  ";
		}
	}
}

void PermanentMagnet::Set_Field2D_Conf(int prec_H, int prec_R, int _size_x, int _size_y)
{
	int x, y, z, h, angle;
	double len;

	Field_2D New_field(_size_x, _size_y);
	Quater_mag_field = New_field;
	cout << "X size of field: " << _size_x << endl;
	Quater_mag_field.size_x = _size_x;
	cout << "Y size of field: " << _size_y << endl;
	Quater_mag_field.size_y = _size_y;
	Quater_mag_field.magnit = this;
	Quater_mag_field.mult = 1;
	...
	for (angle = 0; angle < prec_R; angle++)
	{
		...
		for (h = -prec_H; h < prec_H; h++)
		{
			...
			for (x = 0; x < Quater_mag_field.size_x; x++)
			{
				for (y = 0; y < Quater_mag_field.size_y; y++)
				{
					tmp.set(x, y, 0);
					radius = tmp - koltso_rot;
					len = radius.len();
					Quater_mag_field.cells[x][y].Mag_vec += ((radius * cur_rot)* (1 / (len * len * len)));
				}
			}
		}
	...
	for (x = 0; x < Quater_mag_field.size_x; x++)
	{
		for (y = 0; y < Quater_mag_field.size_y; y++)
		{
			...
			Quater_mag_field.cells[x][y].Mag_vec = Quater_mag_field.cells[x][y].Mag_vec * sight * μ * convert_to_SI;

		}
	}

	FullField Full_field(Quater_mag_field, position);
	Mag_field = Full_field;
	Mag_field.magnit = this;

	cout << "Field configuration is installed  " << endl;
}

Vecter PermanentMagnet::Mag_Mag_Force(PermanentMagnet mag, int H_prec)
{
	//	Vecter force;
	//	Vecter cur;
	//	force.set(0, 0, 0);
	...
	//	double dist = abs((position - mag.position).x_proj);
		...
		//if (ceil((height / 2 + abs((position - mag.position).x_proj))) < mag.Quater_mag_field.size_x)
		//	for (int i = -H_prec / 2; i <= H_prec / 2; i++)
		//	{
			//	if ((i * dheight + (position - mag.position).x_proj) >= 0)
			//	{
			//		force += (cur * (mag.Quater_mag_field.cells[lround(i * dheight + dist)]
	...
	double dheight = height / H_prec;
	if (ceil(height / 2 + abs(dist)) < mag.Quater_mag_field.size_x)
		for (int i = -H_prec / 2; i <= H_prec / 2; i++)
		{
			...
			if ((i * dheight + dist) >= 0)
			{
				dforce1.x_proj = (cur*(mag.Quater_mag_field.cells[_x]
					[lround(radius)].Mag_vec)).x_proj;
				dforce2.x_proj = (cur*(mag.Quater_mag_field.cells[_x - 1]
					[lround(radius)].Mag_vec)).x_proj;
...
Vecter ElectroMagnet::Force_from_coil(PermanentMagnet mag)
{
	Vecter force(0, 0, 0), dforce1(0, 0, 0), dforce2(0, 0, 0);
	Vecter cur;
	int _x;
...
void ElectroMagnet::Set_Field2D_Conf(int prec_R, int _size_x, int _size_y) /* USE FIELD MULTIPLYIER (CURRENT) TO CALCULATE REAL FIELD*/{
	...
	for (x = 0; x < Quater_mag_field.size_x; x++)
	{
		for (y = 0; y < Quater_mag_field.size_y; y++)
		{
			//	Quater_mag_field.cells[x][y].Mag_vec.z_proj = 0;
			Quater_mag_field.cells[x][y].Mag_vec = Quater_mag_field.cells[x][y].Mag_vec * μ * convert_to_SI;
...
double Flow_X_flat(Magnet mag, double distance, int diametr)
{
	...
	if (x < mag.Quater_mag_field.size_x)
	{
		...
		for (y = 1; y <= diametr; y++) {
			if (!((x <= mag.height / 2) && (((y / 2) == mag.diametr / 2)
				...
				prm = (mag.Quater_mag_field.cells[x][y / 2].Mag_vec.x_proj + (mag.Quater_mag_field.cells[x][(y - 1) / 2].Mag_vec.x_proj) / 2);
		...
		for (y = 1; y <= diametr; y++) {
			if (!((x - 1 <= mag.height / 2) && (((y / 2) == mag.diametr / 2)
				...
				prm = (mag.Quater_mag_field.cells[x - 1][y / 2].Mag_vec.x_proj + (mag.Quater_mag_field.cells[x - 1][(y - 1) / 2].Mag_vec.x_proj) / 2);
				tmp += prm * π * (y * y - (y - 1) * (y - 1)) / 4;
			}
			else tmp += 0;
		}
...
void CoilSystem::update(double dtime)
{
}
